...
8. 新建五个视图，后续使用
  ...
  -- view_game_curinfo
  select `card_game`.`title` AS `title`,`card_game`.`starttime` AS `starttime`,`card_game`.`endtime` AS `endtime`,`sys_dict`.`dict_value` AS `type`,`view_game_productnum`.`num` AS `total`,`view_game_hitnum`.`num` AS `hit` from (((`card_game` join `view_game_hitnum` on((`card_game`.`id` = `view_game_hitnum`.`gameid`))) join `view_game_productnum` on((`card_game`.`id` = `view_game_productnum`.`gameid`))) join `sys_dict` on((`card_game`.`type` = `sys_dict`.`dict_key`))) where (`sys_dict`.`dict_type` = 'card_game_type')
  -- view_card_user_hit
  select `g`.`title` AS `title`,`u`.`uname` AS `uname`,`u`.`realname` AS `realname`,`u`.`idcard` AS `idcard`,`u`.`phone` AS `phone`,`p`.`name` AS `name`,`p`.`price` AS `price`,`h`.`hittime` AS `hittime`,`h`.`userid` AS `userid`,`h`.`gameid` AS `gameid`,`h`.`productid` AS `productid`,`ud`.`dict_value` AS `level`,`gd`.`dict_value` AS `type`,`g`.`starttime` AS `starttime`,`g`.`endtime` AS `endtime` from (((((`card_user_hit` `h` join `card_game` `g` on((`h`.`gameid` = `g`.`id`))) join `sys_dict` `gd` on(((`gd`.`dict_key` = `g`.`type`) and (`gd`.`dict_type` = 'card_game_type')))) join `card_user` `u` on((`h`.`userid` = `u`.`id`))) join `sys_dict` `ud` on(((`ud`.`dict_key` = `u`.`level`) and (`ud`.`dict_key` = 'card_user_level')))) join `card_product` `p` on((`h`.`productid` = `p`.`id`)))
  ```
