...
<img src="../images/image-20210329143413713.png" alt="image-20210329143413713" style="zoom:40%;" />
...
* 如果用到了`synchronized` 关键字，在底层编译后的jvm指令中，会有`monitorenter`和`monitorexit `两个指令
	...
	* **monitorexit**：此时获取锁的线程就会对那个对象的monitor的计数器减1，如果有多次重入加锁就会对应多次减1，直到最后，计数器是0。
